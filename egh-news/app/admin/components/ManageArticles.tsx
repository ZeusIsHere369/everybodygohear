...
export default function ManageArticles() {
  ...
  return (
    <div className="mt-10 rounded-xl bg-white p-6 shadow">
      <h2 className="mb-6 text-2xl font-bold">📰 Manage Articles</h2>
        ...
        <table className="min-w-full border-collapse">
          ...
          <tbody>
            {articles.map((article) => (
              <tr
                key={article.id}
                className={`border-b hover:bg-gray-50 ${articleId === article.id ? "bg-yellow-50" : ""}`}
              >
                <td className="p-3 font-semibold">{article.headline}</td>
